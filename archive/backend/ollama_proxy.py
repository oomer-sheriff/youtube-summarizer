...
async def execute_mcp_tool(tool_name: str, args: dict) -> str:
    """Makes a request to the MCP server to execute a tool."""
    try:
        logging.info(f"Calling MCP tool '{tool_name}' with args: {args}")
        response = requests.post(f"{MCP_SERVER_URL}/tools/{tool_name}", json=args, timeout=600)
        response.raise_for_status() # Raise an exception for bad status codes
        result = response.json().get("result", "Tool executed but returned no result.")
        logging.info(f"MCP tool '{tool_name}' returned result of length {len(str(result))}")
        return str(result)
    except requests.exceptions.RequestException as e:
        logging.error(f"Error calling MCP server for tool '{tool_name}': {e}")
        return f"Error: Could not connect to the tool server to execute '{tool_name}'."
